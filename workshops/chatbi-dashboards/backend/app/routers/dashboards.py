...
async def list_dashboards(
    ...
    rows = await session.scalars(
        select(Dashboard)
        .where(Dashboard.participant_id == participant.id)
    ...
    return [
        DashboardOut(
            id=d.id,
            name=d.name,
            items=[ItemOut.model_validate(i, from_attributes=True) for i in d.items],
        )
        for d in rows
    ]
...
async def _load_owned_dashboard(
    did: UUID, participant: Participant, session: AsyncSession
) -> Dashboard:
    d = await session.scalar(
        select(Dashboard)
        .where(Dashboard.id == did, Dashboard.participant_id == participant.id)
        .options(selectinload(Dashboard.items))
    )
    if not d:
