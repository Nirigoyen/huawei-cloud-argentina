...
os.environ["WREN_HOME"] = str(Path(__file__).parent.parent / ".wren")
...
PROJECT = Path(__file__).parent.parent / "wren_projects" / "verify"
...
async def main() -> None:
    ...
    async for ev in stream_chat(
        agent, [HumanMessage(content="How many orders are there by status?")]
    ):
        t = ev.get("type")
...
def test_chat_flow() -> None:
    """Pytest entry point for the chat-flow smoke test.

    Requires the mock OpenAI endpoint on :5050 — start it with
    ``.venv/bin/uvicorn mock_openai:app --port 5050`` in another shell.
    Skipped automatically when the mock is unreachable, so CI without the
    fixture stays green.
    """
    try:
        ...
        pytest.skip("mock OpenAI endpoint on :5050 not running")
    asyncio.run(main())
