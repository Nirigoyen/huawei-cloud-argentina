...
async def main() -> None:
    ...
    async for ev in agent.astream_events({"messages": [HumanMessage(content="How many orders by status?")]}, version="v2"):
        kind = ev.get("event")
        name = ev.get("name", "")
        data = ev.get("data") or {}
        if kind in ("on_tool_start", "on_tool_end", "on_chat_model_start", "on_chat_model_stream", "on_chat_model_end"):
            out = data.get("output")
