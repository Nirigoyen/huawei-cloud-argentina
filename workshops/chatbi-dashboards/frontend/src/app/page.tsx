...
export default function Landing() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      setErr(msg.includes("404") ? "Workshop code not found" : msg);
    } finally {
      setBusy(false);
    }
  }
        ...
        <form onSubmit={submit} className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="ECOM2026"
              className="w-full rounded-lg border border-slate-700 bg-slate-800 text-slate-100 placeholder-slate-500 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-slate-500"
              required
            />
          </div>
          <div>
          ...
          <button
            disabled={busy}
            className="w-full rounded-lg bg-white text-slate-900 py-2 font-medium hover:bg-slate-200 disabled:opacity-50"
          >
